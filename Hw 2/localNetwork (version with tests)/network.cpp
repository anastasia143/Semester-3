...
void Network::runVirus()
{
	...
	int infectedComputer = nextStep();
}

void Network::updateSettingsForNewVirus(int randNumber)
{
	for(int i = 0; i < 16; i++)
		infectedOnThisStep[i] = false;
...
int Network::nextStep()
{
	if(!needForCheck.isEmpty())
	{
															...
															&& (!infectedOnThisStep[number]))
		{
			...
			return number; // Return number of infected computer (NEED FOR TEST)
		}
		else
		{
			needForCheck.removeFirst();
	...
	return 0; // If there are no infected computers - return 0;
}
